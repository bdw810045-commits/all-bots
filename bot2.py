...
def btn(text, callback_data=None, url=None, style=None, copy_text=None):
    ...
    if style:
        b.style = style  # Bot API 9.4

    return b

TOKEN = "8227136088:AAH-VMUnq4Bt3lRe5PERHK41IteGhtwJxbo"
ADMIN_ID = 5990930430
...
def accept_terms(call):
    ...
    first_name = call.from_user.first_name or "عزيزي"
    cursor.execute("INSERT OR IGNORE INTO users (user_id) VALUES (?)", (user_id,))
    conn.commit()
    try:
...
def bonus_menu(call):
    if not bot_active:
        bot.answer_callback_query(call.id, "⛔ الخدمة متوقفة حالياً")
        return
...
def my_request(call):
    ...
    if not row:
        try:
            bot.edit_message_text(
                "📭 مفيش طلبات مسجلة باسمك لحد دلوقتي\n\n💡 اختار عرض من القائمة لتقديم طلب جديد",
                call.message.chat.id,
                call.message.message_id,
            ...
            bot.send_message(call.message.chat.id, "📭 مفيش طلبات مسجلة باسمك لحد دلوقتي")
        return
...
# ========== START PROCESS ==========
@bot.callback_query_handler(func=lambda call: call.data.startswith("start_") and call.data != "start_bot")
...
def handle_photo(message):
    if not bot_active:
        bot.send_message(message.chat.id, "⛔ الخدمة متوقفة حالياً")
        return
...
• الـ ID الخاص بالحساب""",
        ...
        markup.add(btn(
            "📋 انسخ رقم الكاش",
            copy_text=CopyTextButton(cash_number),
            style="primary"
        ))
...
def accept(call):
    if call.from_user.id != ADMIN_ID:
        bot.answer_callback_query(call.id, "⛔ مش مصرح ليك")
        return

    user_id = int(call.data.split("_")[1])

    # تحديث حالة الطلب في قاعدة البيانات
    db("""UPDATE requests SET status=? WHERE id=(
        SELECT id FROM requests WHERE user_id=? ORDER BY id DESC LIMIT 1
    )""", ("تم القبول ✅", user_id))

    # تحديث في الذاكرة لو موجود
    if user_id in user_data:
        user_data[user_id]["status"] = "تم القبول ✅"
...
def reject(call):
    ...
    # تحديث في الذاكرة لو موجود
    if user_id in user_data:
        user_data[user_id]["status"] = "مرفوض ❌"
        user_data[user_id]["locked"] = False
